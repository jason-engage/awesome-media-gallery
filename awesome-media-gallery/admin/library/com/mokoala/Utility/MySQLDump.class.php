	...
	function __construct($db = null, $filepath = 'dump.sql', $compress = false, $hexValue = false){
		...
		if($db_con)
		{
			if(mysql_select_db($config->db->name, $db_con))
			{
				...
				mysql_query("SET time_zone='$offset'", $db_con);
			}
		}
		...
		if ( $num_rows > 0 ) {
			...
			for ($i = 0; $i < $num_rows; $i++) {
				...
				for ($j = 0; $j < $num_fields; $j++) {
					...
					if ( (!empty($hexField[$j]) && !empty($record[$field_name])) && ($hexField[$j] && (@strlen($record[$field_name]) > 0) ) )
  ...
  /**
	...
	*/
	function getSqlKeysTable ($table) {
		...
		if (!empty($fulltext) && is_array($fulltext)) {
