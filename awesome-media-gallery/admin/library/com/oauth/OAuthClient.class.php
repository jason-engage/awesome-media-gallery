	...
	Function Initialize()
	{
		...
		switch($this->server)
		{
			...
			case 'Wordpress':
				...
				if (file_exists(dirname($_SERVER['SCRIPT_FILENAME']) . '/admin/' .'config.ini.php')) {
					$config = parse_ini_file(dirname($_SERVER['SCRIPT_FILENAME']) . '/admin/' .'config.ini.php');
					$wp_url = (!empty($config["site.wordpress.site_url"]))?$config["site.wordpress.site_url"]:'';
					//Check for trailing slash and remove if exists
					$wp_url = rtrim($wp_url, "/");
					$this->oauth_version = '2.0';
					$this->dialog_url = $wp_url . '/oauth/authorize/?response_type=code&client_id={CLIENT_ID}&state={STATE}&scope={SCOPE}';
					$this->access_token_url = $wp_url . '/oauth/token';
				}
				break;
